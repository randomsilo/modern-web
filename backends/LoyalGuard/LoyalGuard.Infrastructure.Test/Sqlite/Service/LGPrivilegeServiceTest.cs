...
{
	public class LGPrivilegeServiceTest
	{
		...
		public void CreateUpdateDeleteFetch()
		{
			...
			// faker
			BrashActionResult<LGPrivilege> serviceResult = null;
			...
			serviceResult = lGPrivilegeService.Create(lGPrivilegeCreateModel);
			Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
			Assert.True(serviceResult.Model.LGPrivilegeId > 0);

			// use model with id
			lGPrivilegeCreateModel = serviceResult.Model;
			...
			serviceResult = lGPrivilegeService.Update(lGPrivilegeUpdateModel);
			Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);

			// delete
			serviceResult = lGPrivilegeService.Delete(lGPrivilegeCreateModel);
			Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
			...
			{
				serviceResult = lGPrivilegeService.Create(f);

				Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
				Assert.True(serviceResult.Model.LGPrivilegeId >= 0);
				ids.Add(serviceResult.Model.LGPrivilegeId);
			}
			...
			foreach(var id in ids)
			{
				...
				serviceResult = lGPrivilegeService.Fetch(model);
				Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
				Assert.True(serviceResult.Model.LGPrivilegeId >= 0);
			}
		}
