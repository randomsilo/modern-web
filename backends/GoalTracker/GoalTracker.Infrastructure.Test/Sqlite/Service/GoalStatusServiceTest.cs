...
{
	public class GoalStatusServiceTest
	{
		...
		public void CreateUpdateDeleteFetch()
		{
			...
			// - service
			var goalStatusService = new GoalStatusService(goalStatusRepository, logger);
			Assert.NotNull(goalStatusService);

			// faker
			BrashActionResult<GoalStatus> serviceResult = null;
			...
			serviceResult = goalStatusService.Create(goalStatusCreateModel);
			Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
			Assert.True(serviceResult.Model.GoalStatusId > 0);

			// use model with id
			goalStatusCreateModel = serviceResult.Model;

			// update
			var goalStatusUpdateModel = goalStatusFaker.GetOne();
			goalStatusUpdateModel.GoalStatusId = goalStatusCreateModel.GoalStatusId;
			serviceResult = goalStatusService.Update(goalStatusUpdateModel);
			Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);

			// delete
			serviceResult = goalStatusService.Delete(goalStatusCreateModel);
			Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
			...
			{
				serviceResult = goalStatusService.Create(f);

				Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
				Assert.True(serviceResult.Model.GoalStatusId >= 0);
				ids.Add(serviceResult.Model.GoalStatusId);
			}
			...
			foreach(var id in ids)
			{
				...
				serviceResult = goalStatusService.Fetch(model);
				Assert.True(serviceResult.Status == BrashActionStatus.SUCCESS, serviceResult.Message);
				Assert.True(serviceResult.Model.GoalStatusId >= 0);
			}
		}
